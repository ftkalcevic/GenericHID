...
void USB_Device_ProcessControlPacket(void)
{
    ...
    {
	UART1_SendChar('R');
	UART1_SendInt( Request );
	UART1_SendChar('.');
	UART1_SendInt( RequestType );
	UART1_SendChar(' ');
    }
	...
	if (((RequestType & CONTROL_REQTYPE_RECIPIENT) != REQREC_OTHER) &&
	    (RequestType & CONTROL_REQTYPE_TYPE) == REQTYPE_Standard)
	{
	    if ( bSerialDebug )
	    {
...
//    uint16_t                 DevDescriptorSize;
    ...
    //  return;
    //
    //if (ConfigNum > DevDescriptorPtr->NumberOfConfigurations)
    //  return;
...
static void USB_Device_GetDescriptor(void)
{
    ...
    {
	UART1_SendChar('D');
	UART1_SendInt( DescriptorIndex );
	UART1_SendChar('.');
	UART1_SendInt( DescriptorType );
	UART1_SendChar(' ');
    }
	...
	{
	    Endpoint_Write_Byte(*DescriptorPointer++);
	    DescriptorBytesRem--;
	}
