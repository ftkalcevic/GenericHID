...
ByteArray MakeEEPROM::makeEEPROM()
{
    ...
    appHeader.nOptions = (m_ConfigConfig->busPowered() ? DEVICE_OPTION_BUS_POWERED : 0 ) |
			 ...
			 (m_ConfigConfig->enableJTAG() ? DEVICE_OPTION_ENABLE_JTAG : 0 );
...
QString MakeEEPROM::MakeIntelHexFormat( int nBaseAddress, ByteArray &eeprom )
{
    QString sb;
	...
	int nAddress = nBaseAddress + i;
        sOutput += HexStr(nAddress);	    // offset
        nChecksum = (byte)( nChecksum + (nAddress & 0xff));
        nChecksum = (byte)( nChecksum + ((nAddress>>8) & 0xff) );
